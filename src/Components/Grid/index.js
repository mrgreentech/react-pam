...
const Grid = ({align, alignContent, direction, wrap, justify, ...rest}) => {
  const control = [];
  if(direction.length) {control.push(`direction:${direction}`)}
  if(wrap.length) {control.push(`wrap:${wrap}`)}
  if(justify.length) {control.push(`justify:${justify}`)}
  if(align.length) {control.push(`align:${align}`)}
  if(alignContent.length) {control.push(`align-content:${alignContent}`)}
...
export default Grid;
export {Unit, Directions, Justify, Wraps};